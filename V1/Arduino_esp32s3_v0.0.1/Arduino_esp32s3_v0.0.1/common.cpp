...
void setup_FFat() {
  if (!FFat.begin(true)) {
    ...
    // 2026-09-11: Report the SPIFFS backend required by the ESP-SR partition instead of the former FFat backend.
    log_error("SPIFFS Initiate Failed.");
    while (true);  // Pause
  } else {
    // log_info("FATFS is Ready.");
    // 2026-09-11: Keep the startup log consistent with the SPIFFS compatibility mapping used for WakeNet.
    log_info("SPIFFS is Ready.");
  }
}
