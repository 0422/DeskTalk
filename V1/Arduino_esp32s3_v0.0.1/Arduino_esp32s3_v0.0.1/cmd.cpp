...
void executeFactoryCommand(String cmd) {
  ...
  // 2026-09-11: Accept the directional calibration commands emitted by the PC client's initial-angle controls.
  } else if (cmd.startsWith("head_left ") ||
             cmd.startsWith("head_right ") ||
             cmd.startsWith("head_up ") ||
             cmd.startsWith("head_down ")) {
    int firstSpaceIndex = cmd.indexOf(' ');
    String direction = cmd.substring(0, firstSpaceIndex);
    String stepString = cmd.substring(firstSpaceIndex + 1);
    stepString.trim();
    bool validStep = !stepString.isEmpty();

    // 2026-09-11: Require an unsigned decimal step so partially numeric text cannot silently alter calibration.
    for (unsigned int i = 0; i < stepString.length(); i++) {
      if (!isDigit(stepString.charAt(i))) {
        validStep = false;
        break;
      }
    }
    ...
    // 2026-09-11: Reject malformed steps and cap each click to ten degrees to reduce collision and brownout risk.
    if (!validStep || step <= 0) {
      log_warn("[Factory] Invalid head calibration step: %s", stepString.c_str());
      return;
    }
    step = constrain(step, 1, 10);

    // 2026-09-11: Map the client labels to the existing signed center calibration convention and persist the result.
    if (direction == "head_left") {
      adjust_x_center(-step);
    } else if (direction == "head_right") {
      adjust_x_center(step);
    } else if (direction == "head_up") {
      adjust_y_center(-step);
    } else {
      adjust_y_center(step);
    }
    head_center(3);
    ...
    reset_wifi();
  } else if (cmd == "speaker_test") {
    test_speaker();
  // 2026-09-11: Add non-blocking serial controls for owner enrollment and face-follow operation.
  } else if (cmd == "enroll_owner") {
    request_owner_enrollment();
  } else if (cmd == "clear_owner") {
    clear_owner_face();
  } else if (cmd == "face_follow_on") {
    set_face_follow_enabled(true);
  } else if (cmd == "face_follow_off") {
    set_face_follow_enabled(false);
  } else if (cmd == "face_status") {
    print_face_tracking_status();
  } else {
